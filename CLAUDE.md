...
- **Subscribe CTA is concatenated, NOT a merge segment**: the ~5s mascot CTA clip (`assets/cta/subscribe_cta.mp4`, 2-beat: 訂閱按钮点击+铃铛叮 → 留言提问) is appended to the **first battle segment** via `scripts/append_cta.sh <battle1>_final.mp4` BEFORE merge (Step 6.5 of the workflow spec), so it plays mid-roll between battles 1 and 2 *inside* battle 1's chapter. Never pass it to merge as its own `--segment` — at <10s it would nuke the whole chapter list. `append_cta.sh` does a filter-level **re-encode** concat (~5 min for a 24-min segment) — stream copy once emitted a backward-pts join that made merge silently drop the segment tail + CTA. Editable sources + regen instructions live in `assets/cta/` (README + `src/`).
