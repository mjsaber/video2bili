...
- `tavern_bg_pastel.png`: current Japanese fresh pastel colored-pencil background,
  generated by built-in ImageGen from the original tavern; full prompt in
  `tavern_bg_pastel.prompt.txt`. Clearly visible mint, powder blue, peach and
  apricot color areas preserve the pencil texture.
- `tavern_bg.png`: earlier mostly graphite ivory-paper tavern background; scene and art
